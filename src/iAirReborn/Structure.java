...
@Manifest(authors = "Bavaria", name = "iAirReborn", description = "Runecrafts air rune for you.. Version V5.02")
public class Structure extends PollingScript implements PaintListener {
			...
			public void run() {
				if (container == null) {
					container = new JobContainer(new Job[] {
							...
							new OpenAltar(getContext()),
							new Crafting(getContext()),
							new WalkToBank(getContext()),
							new LeavingAltar(getContext()), new Failsafe(getContext()) });
	...
	public void repaint(Graphics g1) {
		...
		g.drawString("Trips made : " + Vars.tripsMade + " ("
				+ getPerHour(Vars.tripsMade) + " P/H)", 20, 82);
		g.drawString("Essence Used : " + Vars.essUsed + " ("
				+ getPerHour(Vars.essUsed) + " P/H)", 20, 96);
		g.drawString(
				"Experience gained : "
						+ (ctx.skills.getExperience(Skills.RUNECRAFTING) - Vars.startExp)
						+ " ("
						+ getPerHour(ctx.skills
								.getExperience(Skills.RUNECRAFTING)
								- Vars.startExp) + " P/H)", 20, 109);
